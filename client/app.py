...
if st.session_state.logged_in:
    ...
    if st.sidebar.button("Login"):
        if login_username and login_password:
            ...
            if status_code == 200:
                ...
                st.session_state.auth = HTTPBasicAuth(
                    login_username, login_password)
    ...
    signup_username = st.sidebar.text_input(
        "New Username", key="signup_username")
    signup_password = st.sidebar.text_input(
        "New Password", type="password", key="signup_password")
    signup_role = st.sidebar.selectbox(
        "Role", ["patient", "doctor"], key="signup_role")
    if st.sidebar.button("Signup"):
        if signup_username and signup_password:
            status_code, data = signup_user(
                signup_username, signup_password, signup_role)
            if status_code == 200:
                st.sidebar.success(
                    "User created successfully! You can now log in.")
            elif status_code == 400:
                st.sidebar.error(
                    f"Signup failed: {data.get('detail', 'User already exists')}")
            else:
                st.sidebar.error(
                    f"Signup failed: {data.get('detail', 'Unknown error')}")
        else:
            ...
            if upload_submitted and uploaded_files:
                with st.spinner("Uploading and processing reports..."):
                    status_code, data = upload_report(
                        st.session_state.auth, uploaded_files)
                    if status_code == 200:
                        st.success(
                            f"Reports uploaded successfully! Your Document ID is: `{data['doc_id']}`")
                        # Store doc_id for later use
                        st.session_state.doc_id = data['doc_id']
                    else:
                        st.error(
                            f"Upload failed: {data.get('detail', 'Unknown error')}")
                ...
                if diagnosis_submitted:
                    with st.spinner("Generating diagnosis..."):
                        ...
                        if status_code == 200:
                            ...
                            st.error(
                                f"Diagnosis failed: {data.get('detail', 'Unknown error')}")
        else:
        ...
        with st.form("doctor_form"):
            ...
            if view_submitted:
                with st.spinner(f"Fetching diagnosis records for {patient_name_input}..."):
                    status_code, data = get_doctor_diagnosis(
                        st.session_state.auth, patient_name_input)
                    if status_code == 200:
                        st.success(
                            f"Found {len(data)} diagnosis record(s) for **{patient_name_input}**.")
                        st.markdown("---")
                    if isinstance(data, list):  # ensures we're iterating a list
                        # Loop through each diagnosisf record and format it
                        for record in data:
                            if isinstance(record, dict):  # ensures record is a dict
                                # tell Pylance this is a dictionary
                                record: Dict[str, Any]

                            record_id = record.get("_id", "N/A")
                            doc_id = record.get("doc_id", "N/A")
                            ...
                            # Display record
                            st.info(f"**Diagnosis Record ID:** {record_id}")
                        ...
                        st.error(
                            f"Failed to fetch records: {data.get('detail', 'Unknown error')}")
